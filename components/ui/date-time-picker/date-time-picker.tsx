...
const DateTimePicker = (props: DatePickerProps) => {
  ...
  const onChangeWrapper = (value: ExtendedDateValue, newHasTime?: boolean) => {
    let newvalue = new CalendarDateTime(
      (value && value.year) ||
        (props.value && props.value.date?.getFullYear()) ||
        0,
      (value && value.month) ||
        (props.value && props.value.date?.getMonth()) ||
        0,
      (value && value.day) || (props.value && props.value.date?.getDate()) || 1,
      (value && value.hour) ||
        (props.value && props.value.date?.getHours()) ||
        0,
      (value && value.minute) ||
        (props.value && props.value.date?.getMinutes()) ||
        0,
      (value && value.second) ||
        (props.value && props.value.date?.getSeconds()) ||
        0,
      (value && value.millisecond) ||
        (props.value && props.value.date?.getMilliseconds()) ||
        0
    );

    const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    props.onChange({
      date: newvalue.toDate(timeZone),
